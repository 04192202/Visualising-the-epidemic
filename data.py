...
def get_hotdata():
    ...
    # 浏览器
    brower = Chrome('chromedriver.exe')
    # 加载 url 地址
    brower.get(url)
    time.sleep(8)
    # 加载整个页面的 html 文件
    ...
    btn = brower.find_element_by_xpath('//*[@id="tab"]/div/a[3]')
    # 执行点击操作
    btn.click()
    # 等待加载时间为1秒
    time.sleep(1)
    ...
    # 第二个点击按钮 展开更多
    btn = brower.find_element_by_xpath('//*[@id="ptab-1"]/div[3]/div[11]')
    # 执行点击操作
    btn.click()
    time.sleep(1)

    # 获取数据
    content = brower.find_elements_by_xpath('//div[@data-click="{\'act\': \'news_more_click\'}"]')
    # print(len(content))
    for item in content:
        # print(item.text)
        hotdata.append(item.text.split('\n')[1])  # 只要文本内容
    # 关闭浏览器
    brower.quit()
    return hotdata
